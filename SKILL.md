...
- Prefer `pk-auto-crystallize` after task completion when touched files and a task summary are available
...
- `pk-preflight`: inspect project knowledge before a task and return matching practices or local evidence hints
...
- `pk-lint`: report recommendation-pool lifecycle governance, evidence health, and possible duplicate knowledge without modifying files
- `pk-govern`: automatically apply reversible governance actions such as promotion, demotion, and strong duplicate rejection
...
- `pk-lint` governance findings are review prompts only: promotion candidates, eviction candidates, and possible duplicates still require human judgment before moving or deleting files
