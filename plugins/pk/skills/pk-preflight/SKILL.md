...
- Accept a project root or `.repowise/` directory plus a task description.
...
- If `.repowise/project-profile.md` is missing, report `mode: no-knowledge` and do not scan local code or evidence; suggest `pk-init` only if the user wants this project to opt in.
